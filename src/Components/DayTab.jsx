...
function TodayTab({ className = "", weatherData, conditionData, loadingStage }) {
	console.log('TodayTab',loadingStage);
    ...
    <>
      <div className={`h-fit sm:h-full ${className}`}>
        <div className="h-full flex flex-col sm:grid sm:grid-rows-[auto_1fr] lg:grid-rows-[auto_3fr] gap-5 sm:gap-y-3">
          <Hours loadingStage={loadingStage}></Hours>
          <DayHighlights loadingStage={loadingStage}></DayHighlights>
        </div>
      </div>
    </>
  );
